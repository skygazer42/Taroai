...
Current storage lifecycle implementation has started with `retention_expires_at` and `deleted_at` metadata, a retention-aware object delete API, SQL metadata tombstones, S3/MinIO-compatible delete adapter boundary, `storage.deleted` audit metadata, a first-pass object storage cleanup worker that deletes expired active objects through the adapter and records worker/audit events, and a first-pass `taroai/lifecycle` package with Pydantic lifecycle policy models, in-memory/SQL policy stores, tenant default plus workspace override policy resolution, active legal-hold checks wired into storage cleanup, storage cleanup preview API, storage-object export manifest generation, tenant/workspace/run scoped JSON export bundle upload, safe backup manifest generation, data residency report generation, tenant offboarding request planning with legal-hold blocking, persisted offboarding plans with approval state advancement, approved tenant-scoped offboarding export bundle execution, first-pass offboarding storage-object deletion execution, memory tombstone/delete execution, knowledge metadata/chunk deletion execution, lifecycle policy/legal-hold/export/backup-manifest/data-residency/offboarding APIs behind `lifecycle.read`/`lifecycle.manage`, private install validation restore-drill evidence intake, a restore drill evidence builder that checks backup manifest, executed restore order, database migration plan, object storage verification, Redis queue verification when in scope, config restoration, and post-restore checks, and a first-pass scheduled restore drill due-job scheduler plus due-worker request-record intake with safe audit metadata, disabled-schedule queued-job skip handling, due-job payload validation against the stored schedule with schedule-context failure audit, duplicate due-job idempotency by schedule timestamp, store-level run-record idempotency with SQL uniqueness on `(tenant_id, schedule_id, scheduled_for)`, SQL-backed schedule/run-record persistence through Pydantic settings, and lifecycle API endpoints to create/list/update schedule status, list schedule run records, enqueue requested run-record verifier execution, and close requested run records to `evidence_ready` or `failed` with terminal-state protection plus tenant/workspace/data-export/application-json/unexpired/retrievable non-empty size-matched passed restore-drill-verification evidence storage-object validation, plus first-pass restore-drill execution/evidence workers that consume verifier configs, enqueue governed evidence collection, write successful evidence as managed data-export objects, mark failed verification terminal, and record safe worker audit metadata. Remaining cleanup categories beyond memory, tenant-wide asynchronous export orchestration, backup execution, actual restore environment orchestration, broader offboarding deletion orchestration, and physical cross-region infrastructure remain planned work.
