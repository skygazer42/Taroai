...
def test_create_app_wires_guardrails_into_default_tool_gateway():
    ...
    assert getattr(app.state.long_term_memory_service, "guardrail_service", None) is guardrail_service
    assert getattr(app.state.short_term_memory_service, "guardrail_service", None) is guardrail_service
...
def test_run_events_are_served_as_sse():
    ...
    event_types = [json.loads(line)["type"] for line in data_lines]
    assert event_types == [
        "run.created",
        "billing.metered",
        "audit.recorded",
...
def test_audit_coverage_endpoint_reports_default_matrix_for_current_tenant():
    ...
    assert report.json()["total_requirements"] == 17
...
def test_billing_meters_endpoint_filters_by_run_user_workspace_and_type():
    ...
    by_run = client.get("/api/billing/meters", headers=headers, params={"run_id": first["run_id"]})
    by_user = client.get("/api/billing/meters", headers=headers, params={"user_id": second_auditor.id})
    ...
    assert [meter["run_id"] for meter in by_run.json()] == [first["run_id"], first["run_id"]]
    assert [meter["run_id"] for meter in by_user.json()] == [second["run_id"]]
    assert [meter["workspace_id"] for meter in by_workspace.json()] == ["workspace_support"]
...
def test_run_trace_returns_runtime_stage_spans_for_executed_run():
    ...
    client = TestClient(create_app(identity_service=identity, store=store, runtime=runtime))
    created = client.post(
        "/api/runs",
        headers={"X-Tenant-ID": "tenant_acme", "X-User-ID": auditor.id},
        json={
    ...
    assert {
        span["parent_span_id"]
        for span in runtime_spans.values()
    } == {root_span_id}
    ...
    assert runtime_spans["runtime.artifact"]["attributes"]["artifact_type"] == "document"
...
def test_run_trace_returns_guardrail_findings_without_raw_content():
    ...
    store.record_audit_event(
        ...
        metadata={
            ...
            "guardrail_detector_finding_ids": ["builtin_prompt_threat.prompt_injection"],
            "severity": "high",
...
def test_audit_events_endpoint_filters_by_event_actor_run_workspace_and_date():
    ...
    by_user = client.get("/api/audit-events", headers=headers, params={"user_id": second_auditor.id})
    by_run = client.get("/api/audit-events", headers=headers, params={"run_id": second["run_id"]})
    by_workspace = client.get(
        "/api/audit-events",
        headers=headers,
    ...
    assert [event["event_type"] for event in by_event_type.json()] == ["storage.uploaded"]
...
def test_run_lifecycle_can_use_sql_control_plane_store_from_settings(tmp_path):
    ...
    assert queue.claim(JobType.RUN_EXECUTION, worker_id="agent_worker_1").payload["run_id"] == run_id
...
def test_storage_metadata_and_signed_url_endpoints_are_tenant_scoped():
    ...
    client = TestClient(
        create_app(
            ...
            settings=Settings(object_storage_signed_url_ttl_seconds=900, _env_file=None),
        )
    )
...
def test_storage_endpoints_require_permissions_and_audit_signed_url_creation():
    ...
    storage_audits = [
        event for event in audits.json() if event["event_type"] == "storage.signed_url.created"
    ]
...
def test_storage_delete_respects_retention_and_records_tombstone_audit():
    ...
    assert delete_audits[0]["metadata"]["retention_expires_at"] == expired_retention.isoformat()
...
def test_app_can_use_sql_long_term_memory_from_settings(tmp_path):
    ...
    assert isinstance(app.state.long_term_memory_service.service, SqlLongTermMemoryService)
...
def test_app_can_use_redis_short_term_memory_from_settings():
    ...
    assert isinstance(app.state.short_term_memory_service.service, RedisShortTermMemoryService)
...
def test_memory_api_reviews_candidates_before_active_reads_and_records_audit():
    ...
    assert [record["id"] for record in active_after_review.json()] == [candidate.json()["id"]]
...
def test_memory_api_blocks_guarded_candidate_writes_before_persistence():
    ...
    assert [event["metadata"]["guardrail_rule_ids"] for event in guardrail_audits.json()] == [[rule.id]]
...
def test_memory_api_holds_guarded_candidate_writes_for_review_then_activates():
    ...
    assert [record["id"] for record in active_after_review.json()] == [candidate.json()["id"]]
    assert [event["metadata"]["guardrail_rule_ids"] for event in guardrail_audits.json()] == [[rule.id]]
    assert guardrail_audits.json()[0]["metadata"]["guardrail_action"] == "require_approval"
...
def test_short_term_memory_api_lists_and_deletes_run_entries():
    ...
    assert [entry["key"] for entry in listed_after_delete.json()] == ["tool.last_result"]
...
def test_short_term_memory_api_holds_guarded_writes_for_review_then_activates():
    ...
    assert [entry["key"] for entry in active_after_review.json()] == ["planner.scratchpad"]
...
def test_short_term_memory_api_persists_guarded_reviews_when_control_plane_is_sql(tmp_path):
    identity, account = create_memory_admin_identity()
    guardrail_service = InMemoryGuardrailService()
...
def test_memory_api_blocks_configured_secret_detector_writes_before_persistence():
    ...
    assert "sk-enterprise-secret-token" not in str(guardrail_audits.json()[0]["metadata"])
...
def test_cancel_run_endpoint_cancels_paused_run_and_pending_approval():
    ...
    approvals = client.app.state.store.list_approval_requests("tenant_acme", created["run_id"])
    run_audits = [
        event
        for event in client.app.state.store.list_audit_events("tenant_acme")
