...
export class AgentsUI {
  ...
  renderInspector(loading = false) {
    ...
    const compiledPlaybook = agent.runtime_snapshot?.compiled_playbook;
    const playbookMarkup = compiledPlaybook
      ? `<div class="agent-config-block"><small>Compiled Playbook · 1 deterministic step</small><details><summary>Review sandbox command</summary><pre>${escapeHtml(compiledPlaybook.decision?.tool_input?.command || "Command unavailable")}</pre></details></div>`
      : "";
