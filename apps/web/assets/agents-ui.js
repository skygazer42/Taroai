...
export class AgentsUI {
  ...
  renderCards(searchValue = "") {
    ...
    for (const agent of agents) {
      ...
      card.innerHTML = `<button class="agent-card-main" data-agent-id="${id}"><span class="agent-monogram">${(agent.name || "A").slice(0, 1).toUpperCase()}</span><span><small>${status} · v${version}</small><strong></strong><p></p></span></button><footer><span>${list(agent.skill_bindings, "items").length} skills</span><span>${agent.run_count || 0} runs</span><button data-agent-run-card="${id}">Run</button></footer>`;
  ...
  async select(id) {
    ...
    try {
      const [detail, sessions, evaluations, triggers, activity, files] = await Promise.allSettled([
  ...
  renderInspector(loading = false) {
    ...
    root.innerHTML = `
      <header class="agent-inspector-header"><div><span class="agent-monogram large">${(agent.name || "A").slice(0, 1).toUpperCase()}</span><div><small>${agent.status || "Published"}</small><h2></h2><p></p></div></div><div class="agent-inspector-actions"><button data-agent-export>Export</button><button data-agent-edit>Edit draft</button>${agent.status === "draft" ? `<button class="primary" data-agent-publish-version="${agent.version}">Publish v${agent.version}</button>` : ""}</div></header>
      <nav class="agent-inspector-tabs"><button class="is-active" data-agent-tab="run">Run</button><button data-agent-tab="schedule">Schedule</button><button data-agent-tab="configuration">Configuration</button><button data-agent-tab="evaluation">Evaluation</button><button data-agent-tab="versions">Versions</button><button data-agent-tab="sessions">Sessions</button><button data-agent-tab="activity">Activity</button></nav>
  ...
  async openDraft(agent = {}) {
    const workspace = encodeURIComponent(this.api.settings().workspaceId);
    const [files, browserProfiles, engineConnections, repositories, evaluationSuites, skills, readiness] = await Promise.allSettled([
